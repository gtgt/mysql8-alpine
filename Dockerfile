...
RUN --mount=type=cache,target=/usr/src/mysql/build,sharing=locked \
    ...
    (patch -p1 < /patch/openssl3.patch; patch -p1 < /patch/8.4-bulk_data_service.patch; patch -p1 < /patch/8.0.41-my_safe_puts_stderr.patch; patch -p1 < /patch/8.0.43-my_stacktrace.patch; patch -p1 < /patch/8.0.45-ssl_context_builder.patch; true) && \
